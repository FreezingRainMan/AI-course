...
def main():
    ...
    # gets and loads the checkpoint file from the location as specified by the user as one of the input arguments 
    # if no such argument is found, then the default data file (checkpoint7.pth) is used 
    if in_args['chk']:
        answer = input("Enter a valid checkpoint.pth filename in the current directory")
        model, optimizer = load_checkpoint('./checkpoint9.pth')
    else:
    ...
    train_model(in_args, model, optimizer, trainloader, validationloader)
    ...
    save_checkpoint(in_args, test_set, model, optimizer)    
