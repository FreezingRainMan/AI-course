...
def get_input_args():
    ...
    Command Line Arguments:
      ...
      2. Model architecture as --arch < >                                                                                         default 'resnet50'
                                        possible values of "vgg16", "densenet121", "alexnet", "resnet50", "resnet152"}             
      3. Learning rate as --learn < >                         possible values of (0 < value <= 1)                                 default 0.3
      4. Number of hidden units as --hidden_units < >         possible values of (0 < value)                                      default 1012
      5. Number of epochs as --epochs < >                     possible values of (1 < value <= 50)                                 default 8
      ...
      8. Train using an already trained checkpoint file  --chk < > any valid path & filename ending in .pth                        default "./checkpoint7.pth"
    ...
    parser.add_argument("--learn",        help = "Learning rate - (default  0.3)",                        type = float, default = 0.3)
    parser.add_argument("--hidden_units", help = "Number of hidden units - (default 1012)",               type = int, default = 1012)
    parser.add_argument("--epochs",       help = "Number of epochs - (default of 8)",                     type = int, default = 2)
...
def load_checkpoint(filename):
    ...
    - the ASSOCIATED (WEIGHTS & BIASES) from the model's state_dict
      Parameters:    The checkpoint filename 
      Returns:       The model & optimizer
    ''' 
    ...
    model.fc = checkpoint['classifier']                                # these 2 lines instead of re-creation !!!
    ...
    print("Your checkpoint has now been loaded.")
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------- #
def train_model(in_args, model, optimizer, trainloader, validationloader):
    '''
    ...
    - the ASSOCIATED (WEIGHTS & BIASES) from the model's state_dict
      ...
      Returns:       The trtained model & optimizer
    ''' 
    ...
    print("TRAINING: No. of epochs being used : ", epochs)
    print("TRAINING: Learning rate being used : ", my_learn_rate)
    print("\nNow Training....")
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------- #
def save_checkpoint(in_args, test_set, model, optimizer):
    '''
    This function saves a checkpoint.  It is typically used after training has completed.
      Parameters:    the input arguments, model, optimizer, the test_set
      Returns:       None
    '''
    ...
    checkpoint = {'input_size' : 2048,
                  ...
                  'fc3' : 103,
                  'output_size' : 103,
                  'architecture' : 'resnet50',          # saving this means that you dont need to re-load the model from torchvision models later
    ...
    print("Your trained model is now saved as {}".format(save_dir))
...
def build_model(in_args):
    ...
    print("model is ", in_args['arch'][0:3])
